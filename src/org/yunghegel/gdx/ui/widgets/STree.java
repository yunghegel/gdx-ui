...
//            we want to draw an alternating grid pattern as slots for the nodes
//            first we need to know how many rows we can fit in the tree

            N root = nodes.get(0);
            float rowHeight = 26;

            int numPossibleRows = (int) (getHeight()/rowHeight)-1;
//            modulo for last row height
            float lastRowHeight = getHeight()%rowHeight;



            for (int i=0; i<numPossibleRows; i++) {
...
//                drawRow(root, style.selection, batch, x, rowY, getWidth(), rowHeight,i);
            }
...
//                N node = nodes.get(0);
//                Actor actor = node.actor;
//                actorY = node.height * i;
//                float height = node.height;
//                if(rowBounds!=null){
//                    drawRow(node, style.selection, batch, x, actorY, getWidth(), height + ySpacing, i);
//
////                    drawRow(node, style.selection, batch,rowBounds.x, rowBounds.y*i, rowBounds.width, rowBounds.height,i);
//
//                }
//
//
//            }
    ...
    protected float drawIcons (Batch batch, float r, float g, float b, float a, @Null N parent, Array<N> nodes, float indent,
        ...
        for (int i = 0, n = nodes.size; i < n; i++) {
            ...
            if (cullingArea == null || (actorY + height >= cullBottom && actorY <= cullTop)) {
                ...
                } else if (!rootNodes.contains(node,true)) {
                    drawRow(node, style.selection, batch, x, y + actorY - ySpacing / 2, getWidth(), height + ySpacing,i);
                }
    ...
    protected void drawRow (N node, Drawable selection, Batch batch, float x, float y, float width, float height,int index) {
        if(index%2==0) {
        ...
        style.row.draw(batch, x, y, width, height);
    }
     ...
     * @author Nathan Sweet */
    static abstract public class Node<N extends STree.Node, V, A extends Actor> {
        A actor;
        N parent;
